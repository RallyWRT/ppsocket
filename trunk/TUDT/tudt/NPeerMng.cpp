	...
	case NL_CODE_FAILED:
		{
			...
			{
				if(INVALID_PEER_ID!=m_iServerPeer) //是不是有意不连接服务器？
				{
	...
	case NL_CODE_BREAKDOWN:  //连接中断
		{
			...
			if ( INVALID_PEER_ID!=m_iServerPeer 
				&& id==m_iServerPeer)  //如果是主连接断了，那么自动重连
			{
				ConnectToServer(m_iServerPeer,&m_addrServer,sizeof(m_addrServer));
			}
...
void NPeerMng::OnData( int iConn,int iLen, const char *pData )
{
	...
	if (INVALID_PEER_ID==id) //没有通过认证
	{
		...
		std::cout<<pSendID->uiUserId<<"新连接的本地地址："<<pSendID->addr.ToString()<<std::endl;
	...
	case TBasePack::PT_DATA:
		{
...

			{
				...
				//先复制一份副本，这样遍历的时候用户可以AddListener或者RemoveListener。 
...
bool  NPeerMng::PeerConnect(T_PEERID iPeerId,int iWaitTime)
{
	...
	//////////////////////////////////////////////////////////////////////////
	//tianzuo,2009-8-9. 首先要保证Server是连接状态
	//ConnectToServer(m_iServerPeer,&m_addrServer,sizeof(m_addrServer));
	////等待指定的时间，看连接成功否
...
void NPeerMng::ClosePeer(T_PEERID iPeerId)
{
	...
	ATLTRACE("NPeerMng::ClosePeer耗时%dms\n",GetTickCount()-dwBgn);
}
