...
//////////////////////////////////////////////////////////////////////////
TPeerMng::TPeerMng(T_PEERID iMyId, const sockaddr* name, int namelen)  //系统唯一的节点id和入口服务器地址
{
...
NPeerMng::NPeerMng(TPeerMng *pParentPeerMng,T_PEERID iMyId, const sockaddr* name, int namelen)  //系统唯一的节点id和入口服务器地址
:m_Network(name,namelen),m_iMyId(iMyId),m_pParentPeerMng(pParentPeerMng)
{
	m_bExit = FALSE;
	//modify by gaocheng
	m_connectEvent = CreateEvent(NULL, false, false, NULL);
	m_bExit = CreateEvent(NULL, false, false, NULL);
...
NPeerMng::~NPeerMng()
{
	...
	WaitForSingleObject(m_ExitEvent,1000);
...
//回调函数
TPeerPtr NPeerMng::PeerFactory(T_PEERID iPeerId,int iTemplateType)      //有新连接上来
{
...
void *NPeerMng::Thread() 
{
	while (m_bExit)
	{
		if (m_listNode.size()>0)
		{
			...
			connectNode it = m_listNode.front(); 

			OnConnectionMsgThread(it.iConn,it.code,it.param);
		}
		WaitForSingleObject(m_connectEvent,10*1000);
	}
...
void NPeerMng::OnConnectionMsgThread( int iConn,enNetworkCode code,int param )
{
	switch(code)
	{
	...
	case NL_CODE_BREAKDOWN:  //连接中断
		{
			...
			T_PEERID id = Socket2Peer(iConn); //这一步必须在RemoveSockID.erase之前
			RemoveSockID(iConn);
