...
{
public:
	NPeerMng(UDT::TPeerMng *pParentPeerMng,unsigned int iMyId, const sockaddr* name, int namelen);  //系统唯一的节点id和入口服务器地址
	virtual ~NPeerMng();
	...
	//根据用户ID查找用户
	UDT::TPeerPtr FindPeer(unsigned int iPeerId);
	...
	UDT::TPeerPtr GetPeerInstance(int iPeerId,int iTemplateType);
	//关闭连接。此函数关闭所有相关网络连接，释放对应智能指针
	void ClosePeer(unsigned int iPeerId);

private:
	//用户对象工厂函数：仅供重载使用
	virtual UDT::TPeerPtr PeerFactory(int iPeerId,int iTemplateType); 
	...
	//从用户ID到NPeer的映射表
	typedef std::map<unsigned int, UDT::TPeerPtr> T_mapIDPeer;
...
private:
	...
	//设置id-->NPeer的映射
	bool AddPeer(unsigned int iPeerId, UDT::TPeerPtr pPeer);
